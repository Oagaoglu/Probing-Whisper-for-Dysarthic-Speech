...
# Variable to switch between severity levels
severity_level = 'Very low'  # Change this to 'Normal', 'Medium', 'Low', or 'Very low' as needed
...
# Setup logging
logging.basicConfig(filename=f'training_{severity_level.lower()}.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
def generate_random_representations(num_samples, num_features=1024):
    return pd.DataFrame(np.random.rand(num_samples, num_features))
...
def main():
    ...
    severities = [ "Medium","Very low", "Low"]  # Change this to 'Normal', 'Medium', 'Low', or 'Very low' as needed
    ...
    for severity_level in severities:
        ...
        experimental_acoustic_file = './data_folder/block_2/Low.csv'  # Experimental acoustic file for sample size
        ...
        results_file = f'results_random_{severity_level}.csv'
        ...
        for feature in acoustic_features:
            ...
            # Load acoustic data
            acoustic_data = load_data(f'./data_folder/block_2/{severity_level}.csv',acoustic_file, sample_size=sample_size)
            acoustic_data = preprocess_data(acoustic_data)
            
            # Generate random representations
            X = generate_random_representations(num_samples=len(acoustic_data))
            y = acoustic_data[[feature]]
            ...
            with torch.no_grad():
                ...
                logging.info(f"Test Loss for {feature} from: {test_loss:.4f}")
